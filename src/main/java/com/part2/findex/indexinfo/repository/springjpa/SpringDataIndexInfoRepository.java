...
public interface SpringDataIndexInfoRepository extends JpaRepository<IndexInfo, Long> {
    @Query("SELECT i FROM IndexInfo i WHERE i.indexInfoBusinessKey.indexClassification LIKE :indexClassification AND i.indexInfoBusinessKey.indexName LIKE :indexName AND (:favorite IS NULL OR i.favorite = :favorite)")
